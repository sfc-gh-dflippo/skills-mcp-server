...
server.addResource({
  ...
  load: async () => {
    const text = fs.readFileSync(path.join(__dirname, "resources", "sync-skills.py"), "utf-8");
    return {
      text,
      mimeType: "text/x-python",
      uri: "script://sync-skills.py",
...
server.addResource({
  ...
  load: async () => {
    const text = fs.readFileSync(path.join(__dirname, "resources", "sync-skills.ts"), "utf-8");
    return {
      text,
      mimeType: "application/typescript",
      uri: "script://sync-skills.ts",
...
server.addResource({
  ...
  load: async () => {
    const text = fs.readFileSync(path.join(__dirname, "resources", "manage-repositories.md"), "utf-8");
    return {
      text,
      mimeType: "text/markdown",
      uri: "doc://manage-repositories",
...
server.addPrompt({
  ...
  load: async () => {
    const content = fs.readFileSync(path.join(__dirname, "resources", "manage-repositories.md"), "utf-8");
    return content;
  },
});
