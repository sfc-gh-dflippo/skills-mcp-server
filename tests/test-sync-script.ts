...
async function runTests() {
  ...
  await test("Script has required functions", () => {
    const requiredFunctions = ["readRepoList", "scanAllSkills", "updateAgentsMd", "main"];

    for (const func of requiredFunctions) {
      assert(scriptContent.includes(`function ${func}(`), `Missing required function: ${func}`);
    }
  });
  ...
  await test("Script handles frontmatter parsing", () => {
    assert(scriptContent.includes("frontmatter") || scriptContent.includes("parseFrontmatter"), "Script should handle frontmatter parsing");
  });
  ...
  await test("Script compiles to JavaScript", () => {
    try {
      ...
      execSync(`npx tsc "${syncScriptPath}" --outDir "${tempDir}" --skipLibCheck`, {
        cwd: path.join(__dirname, ".."),
        stdio: "pipe",
      });
